...
describe("AG Pay plugin configuration", () => {
  ...
  it.each([
    ...
    { defaultCheckoutUrl: "https://checkout.stripe.com/c/pay/cs_test_Config123#fragment" },
    {
      defaultCheckoutAdapter: "stripe-hosted",
      defaultCheckoutUrl: "https://checkout.stripe.com/c/pay/cs_test_Config123#fragment",
    },
  ])("rejects removed managed-checkout configuration (%j)", (value) => {
    expect(() => parsePluginConfig(value)).toThrow(/unsupported.*configuration field/i);
  });
